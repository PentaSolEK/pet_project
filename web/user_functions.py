...
from data.concerts import get_concert_by_name, get_concert_by_musicgroup, get_concert_by_id
...
async def get_concert_by_group_name(musicgroup: str, session: SessionDep) -> list[Concert]:
    ...
    concerts_id = await get_concert_by_musicgroup(musicgroup_id, session)
    if not concerts_id:
        raise HTTPException(status_code=404, detail=f"Концерты музыкальной группы {musicgroup} не найдены!")
    list_concerts: list[Concert] = []
    for id in concerts_id:
        list_concerts.append(await get_concert_by_id(id.id_concert, session))
    return list_concerts